...
description: IEEE Xplore research assistant. Coordinates paper search, detail extraction, journal browsing, PDF download, citation export, and IEEE standards catalog search via Chrome DevTools MCP. Use for orchestration across ieee-* workflows.
---
...
**Companion skills (global, `.cursor/skills/`):** For step-level procedures, read each `SKILL.md` as needed — `ieee-search`, `ieee-advanced-search`, `ieee-parse-results`, `ieee-navigate-pages`, `ieee-paper-detail`, `ieee-journal-browse`, `ieee-download`, `ieee-export`, `ieee-standards-search`.
